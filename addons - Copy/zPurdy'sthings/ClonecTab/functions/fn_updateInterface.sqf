		...
		if (_x select 0 == "nightMode") exitWith {
			...
			_background = call {
				if (_displayName in ["cTab_TAD_dsp","cTab_TAD_dlg"]) exitWith {
					if (_nightMode) then {"\ClonecTab\img\TAD_background_night_ca.paa"} else {"\ClonecTab\img\TAD_background_ca.paa"};
				};
				if (_displayName in ["cTab_Android_dsp","cTab_Android_dlg"]) exitWith {
					if (_nightMode) then {"\ClonecTab\img\android_background_night_ca.paa"} else {"\ClonecTab\img\android_background_ca.paa"};
				};
				if (_displayName in ["cTab_microDAGR_dsp","cTab_microDAGR_dlg"]) exitWith {
					if (_nightMode) then {"\ClonecTab\img\microDAGR_background_night_ca.paa"} else {"\ClonecTab\img\microDAGR_background_ca.paa"};
				};
				if (_displayName in ["cTab_Tablet_dlg"]) exitWith {
					if (_nightMode) then {"\ClonecTab\img\tablet_background_night_ca.paa"} else {"\ClonecTab\img\tablet_background_ca.paa"};
				};
				""
			};
			if (_background != "") then {
